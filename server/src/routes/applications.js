...
function pickApplicantEmail(region, applicantType, formData) {
  ...
  try {
    ...
    if (t === "corporate") {
      ...
      if (r === "foreign") return formData?.fcClientRegistration?.emailAddress || "";
    }
  } catch {
    // ignore
  }
...
function buildClientRegistrationRows(region, applicantType, formData) {
  ...
  if (t === "individual" && r === "local") {
    ...
    push("Name", [p.title, p.namesByInitials, p.surname].filter(Boolean).join(" "));
    push("NIC", p.nic);
    ...
    push("CDS", [p.cds?.prefix, p.cds?.number].filter(Boolean).join(" "));
