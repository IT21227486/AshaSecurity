...
function getLiveError(dial, number, countryName) {
  ...
  const phone = parsePhoneNumberFromString(`${dial}${digits}`);
  if (!phone) return `Invalid phone number for ${countryName}.`;

  // During typing: "possible" is a nice check (length-based + basic rules)
  if (!phone.isPossible()) return `Invalid phone number for ${countryName}.`;
...
export default function PhoneInput({
  ...
  const setDial = (dial) => {
    const next = parsed.number ? `${dial} ${parsed.number}` : "";
    onChange?.(next);
  };
  ...
  const liveError = useMemo(
    () => getLiveError(parsed.dial, parsed.number, parsed.name),
    [parsed.dial, parsed.number, parsed.name]
  );
  ...
  return (
    ...
    >
      <div className="flex gap-3">
        <Select className="w-56" value={parsed.dial} onChange={(e) => setDial(e.target.value)}>
          {countries.map((c) => (
        ...
        <Input
          ...
          className={liveError ? "border-red-500 focus:border-red-400" : ""}
        />
      </div>
