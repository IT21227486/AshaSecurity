...
export default function SignUp() {
  ...
  async function onSubmit(e) {
    ...
    setBusy(true);
    try {
      await auth.signup({ name, email, password, tel });
          ...
          <div className="p-7 sm:p-8">
            ...
            <form className="mt-6 space-y-4" onSubmit={onSubmit}>
              ...
              <div>
                ...
                <input className={cx("mt-1 w-full rounded-xl border px-3 py-3 outline-none focus:ring-2 focus:ring-white/20", ui.input)}
                  value={tel} onChange={(e) => setTel(e.target.value)} placeholder="" required />
              </div>
              <div>
                <label className={cx("text-xs font-semibold", ui.muted)}>Email</label>
                <input className={cx("mt-1 w-full rounded-xl border px-3 py-3 outline-none focus:ring-2 focus:ring-white/20", ui.input)}
                  value={email} onChange={(e) => setEmail(e.target.value)} type="email" placeholder="abc@example.com" required />
              </div>
              <div>
